...
/*    public void bind(CampusMcpClient mcp, RagService_old rag) {
        this.mcp = mcp;
        this.rag = rag;
        setEnabled(true);
        if (rag == null) {
...
*/
    ...
    private Tab bookingTab() {
        ...
        form.setHgap(10);
        form.setVgap(8);
        resourceField.setPromptText("KA-P1");
    ...
    private void wire() {
        askButton.setOnAction(e -> {
            ...
            worker.submit(() -> {
                try {
                    //RagService_old.RagResult r = rag.ask(q, topicField.getText().trim());
